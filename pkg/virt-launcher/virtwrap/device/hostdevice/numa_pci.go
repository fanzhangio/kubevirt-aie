...
func readPCIeLinkWidthForBDF(bdf string) string {
	for _, attr := range []string{"max_link_width", "current_link_width"} {
		data, err := readPCIDeviceFileFunc(filepath.Join("/sys/bus/pci/devices", bdf, attr))
		if err != nil {
			continue
		}
...
// but EGM is only supported when the VM includes the full host GPU group that
// shares the device. Per-GPU EGM size is therefore derived from the full host
// EGM group, not from the VM-local subset of selected GPUs.
//
...
func applyEGMMemoryDevices(domain *api.Domain, devicesWithNUMA []deviceNUMAInfo, graceTopology graceTopologyPlan) error {
	...
	for _, egm := range egmDevices {
		...
		if selectedGPUs != hostGPUs {
			return fmt.Errorf("egm requested for %d/%d GPUs sharing %s; partial EGM configurations are unsupported",
