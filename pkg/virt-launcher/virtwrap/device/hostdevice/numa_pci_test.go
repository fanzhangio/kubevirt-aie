...
func TestApplyNUMAHostDeviceTopologyUsesDedicatedPXBForLargeMMIODevices(t *testing.T) {
	...
	vmi.Annotations = map[string]string{
		v1.GraceVirtualizationAnnotation: `{"hostDevices":true}`,
	}
...
func TestApplyNUMAHostDeviceTopologyAcceptsGraceVirtualizationAnnotationWithSMMUv3Fields(t *testing.T) {
	...
	vmi.Annotations = map[string]string{
		v1.GraceVirtualizationAnnotation: `{"hostDevices":true,"smmuv3":true,"vcmdq":true,"egm":false,"numaStrictLocality":false}`,
	}
...
func TestApplyNUMAHostDeviceTopologyDisablesHostDeviceInjectionWhenGraceVirtualizationHostDevicesIsFalse(t *testing.T) {
	defer restoreNUMAHelpers()
	...
	vmi.Annotations = map[string]string{
		v1.GraceVirtualizationAnnotation: `{"hostDevices":false}`,
	}
	...
	dev := domain.Spec.Devices.HostDevices[0]
	if dev.Driver != nil {
		t.Fatalf("expected iommufd injection disabled by graceVirtualization annotation, got %+v", dev.Driver)
	}
	if dev.ACPI != nil {
		t.Fatalf("expected ACPI nodeset injection disabled by graceVirtualization annotation, got %+v", dev.ACPI)
	}
}
...
func TestApplyNUMAHostDeviceTopologyInjectsSMMUv3IOMMUsAndVCMDQ(t *testing.T) {
	...
	vmi.Annotations = map[string]string{
		v1.GraceVirtualizationAnnotation: `{"hostDevices":true,"smmuv3":true,"vcmdq":true}`,
	}
...
func TestApplyNUMAHostDeviceTopologyInjectsSMMUv3IOMMUsWithoutVCMDQByDefault(t *testing.T) {
	...
	vmi.Annotations = map[string]string{
		v1.GraceVirtualizationAnnotation: `{"hostDevices":true,"smmuv3":true}`,
	}
...
func TestApplyEGMMemoryDevicesRejectsPartialSharedEGMGroup(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "partial EGM configurations are unsupported") {
		t.Fatalf("unexpected error: %v", err)
	}
}
